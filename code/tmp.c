    ...
    for (int step = 0; step < STEPS; step++){
        ...
        for (int round = 0; round < ROUNDS_NB; round++) {
            ...
            if (is_great_word == 0 && sum_words == (WORDS_NB - 1)) {
                is_great_word = 1;
                the_best_index = get_target_array_index(0, words, WORDS_NB);
                char * guess_word = all_W_and_S[the_best_index].word;
                printf("GUESS %s\n", guess_word);

                // Init a,b and c
                // TODO : ne fonctionne pas
                // set_a_b_c_variables(&a, &b, &c, save_concepts_indexes, the_best_index);
            } else printf("PASS\n");
            fflush(stdout); // debug
